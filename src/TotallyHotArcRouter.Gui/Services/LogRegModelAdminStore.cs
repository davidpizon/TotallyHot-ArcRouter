...
public sealed class LogRegModelAdminStore : IDisposable
{
    ...
    public async Task LoadAsync(CancellationToken cancellationToken = default)
    {
        ...
        catch (LogRegModelAdminException ex)
        {
            ...
            _logger?.LogWarning(ex, "Failed to load the logreg model status from the router.");
        }
        finally
        {
            IsLoaded = true;
    ...
    private void RecordFailure(LogRegModelAdminException ex)
    {
        ...
        IsReachable = false;
        LastError = ex.Message;
        _logger?.LogWarning(ex, "The router became unreachable during a logreg-model operation.");
        Changed?.Invoke();
    }
