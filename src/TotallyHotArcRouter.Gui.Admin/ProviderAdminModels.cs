...
/// <param name="Name">
/// The user-friendly display name for this provider (e.g. <c>OpenAI API</c>); displayed in the
/// Governance UI.
/// </param>
...
/// <param name="IsFree">
/// Whether this provider costs nothing (a local runtime, say), making its models' cost a known zero
/// rather than unknown.
/// </param>
...
/// The window the caps above reset on: <c>"Monthly"</c>, <c>"Weekly"</c>, or
/// <c>"RollingHours"</c> (Phase 4, §5.10).
/// </param>
/// <param name="NextResetUtc">
/// The UTC instant the current period ends and spend resets to zero - backs a budget bar's
/// "resets in" text.
/// </param>
...
/// <param name="Kind">
/// The classification of a live-traffic failure, or <see cref="ProviderInteractionKindAdminView.None"/>
/// for an AdminAction-track record.
/// </param>
...
/// <summary>
/// A provider's own reported per-model daily token usage (docs/router/secrets-at-rest-plan.md §8.1), mirroring
/// the proxy's <c>ProviderReportedUsageView</c>.
/// </summary>
...
public sealed record ProviderReportedUsageAdminView(
    IReadOnlyList<ReportedUsageRowAdminView> Rows,
    DateTimeOffset FetchedAtUtc);

/// <summary>
/// One (day, model) row of <see cref="ProviderReportedUsageAdminView.Rows"/>. Raw reported counts - no derived
/// totals.
/// </summary>
...
/// What's left before the cap, or <see langword="null"/> if unknown. Anthropic rounds this to the
/// nearest thousand.
/// </param>
...
/// Standard-family dimensions keyed by name (<c>requests</c>, <c>input-tokens</c>,
/// <c>output-tokens</c>, <c>tokens</c>).
/// </param>
...
/// <summary>
/// A provider's most recently captured rate-limit header snapshot, mirroring the proxy's
/// <c>ProviderRateLimitView</c>.
/// </summary>
...
/// <param name="IsStale">
/// Whether <see cref="ObservedAtUtc"/> is older than the proxy's configured staleness threshold
/// (§5.9) - the card dims and labels the "As of" footer when set.
/// </param>
/// <param name="Projections">
/// Per-dimension projected time-to-exhaustion, keyed by the same names as
/// <see cref="RateLimitSnapshotAdminView.StandardDimensions"/>. A dimension absent here has no projection to show.
/// </param>
...
/// <summary>
/// A projected time-to-exhaustion for one rate-limit dimension, mirroring the proxy's
/// <c>RateLimitExhaustionProjection</c>.
/// </summary>
...
/// <summary>
/// One dimension history point for the Providers card's rate-limit trend chart, mirroring the proxy's
/// <c>RateLimitHistoryPointView</c>.
/// </summary>
...
public sealed record RateLimitHistoryResponseAdminView(
    IReadOnlyDictionary<string, IReadOnlyList<RateLimitHistoryPointAdminView>> Dimensions);
...
{
    /// <summary>
    /// A literal value is stored. Returned by the management API only when the header is unlocked
    /// (see <see cref="ProviderHeaderView.Locked"/>); a locked header reports this marker alone.
    /// </summary>
    public const string Literal = "literal";

    /// <summary>
    /// The value is read at request time from an environment variable (see
    /// <see cref="ProviderHeaderView.ValueEnvVar"/>).
    /// </summary>
...
/// </param>
/// <param name="Value">
/// The literal value of an unlocked literal header; null when <paramref name="Locked"/>
/// is set or the value comes from anywhere else.
/// </param>
/// <param name="Locked">Whether this header's value is a secret the router withholds from this GUI.</param>
public sealed record ProviderHeaderView(
    string Name,
    string Source,
    string? ValueEnvVar,
    string? Value = null,
...
/// <param name="Enabled">
/// Whether the provider is switched on; null keeps the existing value. Prefer the
/// dedicated <see cref="ProviderEnabledWriteRequest"/> route for a pure on/off toggle.
/// </param>
...
/// <param name="WindowKind">
/// The window the caps reset on (<c>"Monthly"</c>, <c>"Weekly"</c>, or <c>"RollingHours"</c>);
/// null keeps today's window.
/// </param>
...
/// How many of <paramref name="Requests"/> had an unknown cost (§5.6) - never silently
/// folded into a zero total.
/// </param>
...
/// Meaningful only when the query grouped by <c>"day"</c>; otherwise the query's <c>from</c>
/// bound.
/// </param>
/// <param name="BucketWidth">The ISO-8601 duration the query was made at (<c>"PT1H"</c> or <c>"P1D"</c>).</param>
/// <param name="GroupKey">
/// The model name, provider key, or ISO day string this row aggregates, depending on the query's
/// <c>groupBy</c>.
/// </param>
...
/// The model the frozen baseline would have chosen, or <see langword="null"/> when it
/// abstained.
/// </param>
...
/// The estimated cost of the baseline's pick, or <see langword="null"/> when not
/// estimable.
/// </param>
