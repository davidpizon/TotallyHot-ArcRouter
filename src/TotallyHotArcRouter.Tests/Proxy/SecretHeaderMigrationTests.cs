...
/// Windows-only, since the store itself refuses to write elsewhere -
/// <see cref="Constructor_NoSecretWriter_LeavesTheLiteralInPlace"/>
...
public sealed class SecretHeaderMigrationTests : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        if (secretStoreDirectory is not null && Directory.Exists(secretStoreDirectory))
            Directory.Delete(path: secretStoreDirectory, true);
    }

    private static ModelRoutingOptions OptionsWithLockedLiteral()
    {
    ...
    private ProviderConfigStore CreateStore(ISecretWriter? secretWriter)
    {
    ...
    public void Constructor_ExistingFileWithLockedLiteral_MigratesItToTheStore()
    {
        ...
        File.WriteAllText(path: _tempPath,
            contents: JsonSerializer.Serialize(value: OptionsWithLockedLiteral(),
        ...
        Assert.True(secretStore.TryRead(name: apiKeyHeader.ValueSecretRef!, value: out var migratedValue));
        Assert.Equal(expected: "sk-ant-secret", actual: migratedValue);

        // The unlocked public header must be left exactly as it was.
        var versionHeader = headers.Single(h => h.Name == "anthropic-version");
        Assert.Equal(expected: "2023-06-01", actual: versionHeader.Value);
        ...
        Assert.DoesNotContain(expectedSubstring: "sk-ant-secret", actualString: onDisk,
    ...
    public void Constructor_CalledTwice_IsIdempotent()
    {
        ...
        var refAfterFirst = first.Snapshot.Options.Providers["anthropic"].Headers.Single(h => h.Name == "x-api-key")
            .ValueSecretRef;
        ...
        var refAfterSecond = second.Snapshot.Options.Providers["anthropic"].Headers.Single(h => h.Name == "x-api-key")
            .ValueSecretRef;

        Assert.Equal(expected: refAfterFirst, actual: refAfterSecond);
        Assert.True(secretStore.TryRead(name: refAfterSecond!, value: out var value));
    ...
    public void Constructor_NoSecretWriter_LeavesTheLiteralInPlace()
    {
        ...
        Assert.Equal(expected: "sk-ant-secret", actual: apiKeyHeader.Value);
