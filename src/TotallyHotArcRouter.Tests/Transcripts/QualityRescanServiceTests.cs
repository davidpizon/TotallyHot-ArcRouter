...
public class QualityRescanServiceTests
{
    ...
    public async Task SweepAsync_NoPendingRows_GradesNothing()
    {
        ...
        var service = CreateService(store, grader);
    ...
    public async Task SweepAsync_GradesTheSavedRowAndStampsTheScoreAndVersion()
    {
        ...
        Assert.Equal(1, grader.CallCount);
        var mark = Assert.Single(store.Marks);
        Assert.Equal(7, mark.TranscriptId);
        Assert.Equal("v-test", mark.ScorerVersion);
        Assert.Equal(0.75, mark.Score!.Value, 6);
    }
    ...
    public async Task SweepAsync_PassesTheSavedPromptToTheGrader()
    {
        ...
        Assert.Equal("sort a list in place", grader.LastRequest!.Prompt);
    }
    ...
    public async Task SweepAsync_NullPromptText_PassesEmptyRatherThanFailing()
    {
        ...
        Assert.Equal(string.Empty, grader.LastRequest!.Prompt);
    }
    ...
    public async Task SweepAsync_RecoversTheSessionIdFromTheCorrelationId()
    {
        ...
        Assert.Equal("sess:with:colons", grader.LastRequest!.SessionId);
        Assert.Equal("sess:with:colons:12", grader.LastRequest.CorrelationId);
    }
    ...
    public async Task SweepAsync_MissingRow_StampsNothing()
    {
        ...
        var store = new FakeStore(pending: [7], record: null);
        var grader = new RecordingGrader();
        var service = CreateService(store, grader);
    ...
    public async Task SweepAsync_AnyGateOff_IsANoOp(bool rescanEnabled, bool captureEnabled, bool qualityEnabled)
    {
        var store = new FakeStore(pending: [7], record: MakeRecord(7));
        var grader = new RecordingGrader();
        var service = CreateService(store, grader, rescanEnabled, captureEnabled, qualityEnabled);
    ...
    public async Task SweepAsync_RequestsOnlyTheConfiguredBatchSize()
    {
        var store = new FakeStore(pending: [7], record: MakeRecord(7));
        var grader = new RecordingGrader();
        var service = CreateService(store, grader, batchSize: 25);

        await service.SweepAsync(TestContext.Current.CancellationToken);

        Assert.Equal(25, store.LastRequestedLimit);
        Assert.Equal("v-test", store.LastRequestedScorerVersion);
    }
        ...
        int batchSize = 100)
    {
        ...
        return new QualityRescanService(
            NullLogger<QualityRescanService>.Instance,
            store,
            new Quality.Extraction.CodeBlockSignalExtractor(
                new Quality.Extraction.KeywordDimensionInferrer(),
                Options.Create(qualityOptions),
                NullLogger<Quality.Extraction.CodeBlockSignalExtractor>.Instance),
            grader,
    ...
    private sealed class FakeStore(IReadOnlyList<long>? pending = null, TranscriptRecord? record = null) : ITranscriptStore
    {
