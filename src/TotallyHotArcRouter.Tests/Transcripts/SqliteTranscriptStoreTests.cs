...
public class SqliteTranscriptStoreTests : IDisposable
{
    ...
    [Fact]
    public async Task InsertAsync_RoundTripsTheUntrainedBaselineCounterfactual()
    {
        var database = CreateDatabase();
        database.EnsureCreated();
        var store = new SqliteTranscriptStore(database: database,
        ...
        var id = await store.InsertAsync(
            record: MakeRecord("corr-untrained") with { UntrainedBaselineModel = "glm-5" },
            cancellationToken: TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task InsertAsync_UntrainedBaselineAbstained_StoresNullRatherThanTheServedModel()
    {
        var database = CreateDatabase();
        database.EnsureCreated();
        var store = new SqliteTranscriptStore(database: database,
        ...
        // An abstention means the untrained baseline expressed no preference. Defaulting it to the served
        // model would fabricate a zero-savings counterfactual out of a decision nobody made.
        var id = await store.InsertAsync(record: MakeRecord("corr-untrained-abstain"),
            cancellationToken: TestContext.Current.CancellationToken);

        var row = await store.GetTranscriptAsync(id: id!.Value,
            cancellationToken: TestContext.Current.CancellationToken);
        Assert.Null(row!.UntrainedBaselineModel);
    }
    ...
    public async Task EnsureCreated_DatabasePredatingTheFrozenBaselineCorrection_GainsTheUntrainedBaselineColumn()
    {
        ...
        // correction already exists with the older shape (dim_best_model present, but not the new
        // column) - without the explicit PRAGMA migration every read would fail with "no such column".
        ...
        var id = await store.InsertAsync(
            record: MakeRecord("corr-untrained-migrated") with { UntrainedBaselineModel = "glm-5" },
            cancellationToken: TestContext.Current.CancellationToken);
