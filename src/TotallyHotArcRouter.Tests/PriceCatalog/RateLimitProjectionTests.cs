...
public class RateLimitProjectionTests
{
    ...
    public void Project_NormalDepletion_ReturnsProjection()
    {
        ...
        var earlier = new RateLimitObservation(Earlier, Remaining: 60_000, ResetAt: null);
        var later = new RateLimitObservation(Later, Remaining: 50_000, ResetAt: null);

        var projection = RateLimitProjection.Project(earlier, later);

        Assert.NotNull(projection);
        Assert.Equal(1000d, projection!.BurnRatePerMinute, precision: 3);
        Assert.Equal(TimeSpan.FromMinutes(50), projection.TimeToExhaustion);
    }
    ...
    {
        var earlier = new RateLimitObservation(Earlier, Remaining: 60_000, ResetAt: null);
        var later = new RateLimitObservation(Later, Remaining: 60_000, ResetAt: null);

        Assert.Null(RateLimitProjection.Project(earlier, later));
    }
    ...
    public void Project_Refilled_ReturnsNull()
    {
        ...
        var earlier = new RateLimitObservation(Earlier, Remaining: 10_000, ResetAt: null);
        var later = new RateLimitObservation(Later, Remaining: 60_000, ResetAt: null);
    ...
    public void Project_ResetBeforeEmpty_ReturnsNull()
    {
        ...
        var earlier = new RateLimitObservation(Earlier, Remaining: 60_000, ResetAt: null);
        var later = new RateLimitObservation(Later, Remaining: 50_000, ResetAt: Later.AddMinutes(5));
    ...
    public void Project_ResetAfterProjectedExhaustion_StillReturnsProjection()
    {
        ...
        var earlier = new RateLimitObservation(Earlier, Remaining: 60_000, ResetAt: null);
        var later = new RateLimitObservation(Later, Remaining: 50_000, ResetAt: Later.AddMinutes(60));

        Assert.NotNull(RateLimitProjection.Project(earlier, later));
    }

    [Fact]
    public void Project_EitherRemainingUnknown_ReturnsNull()
    {
        var earlier = new RateLimitObservation(Earlier, Remaining: null, ResetAt: null);
        var later = new RateLimitObservation(Later, Remaining: 50_000, ResetAt: null);

        Assert.Null(RateLimitProjection.Project(earlier, later));
        Assert.Null(RateLimitProjection.Project(later, earlier with { Remaining = null }));
    }
    ...
    public void Project_ObservationsOutOfOrder_ReturnsNull()
    {
        ...
        var earlier = new RateLimitObservation(Later, Remaining: 60_000, ResetAt: null);
        var later = new RateLimitObservation(Earlier, Remaining: 50_000, ResetAt: null);
