...
public class OpenRouterPriceSourceClientTests
{
    ...
    public void Normalize_ConvertsPerTokenStringsToPerMillion_AndSplitsIdIntoProvider()
    {
        ...
        var claude = Assert.Single(prices, p => p.ModelIdentifier == "anthropic/claude-opus-4");

        Assert.Equal("anthropic", claude.Provider);
        Assert.Equal(15.0m, claude.StandardInputPrice);
        Assert.Equal(75.0m, claude.StandardOutputPrice);
        Assert.Equal(1.5m, claude.CachedInputPrice);
    }
    ...
    public void Normalize_NeverProducesBatchRates()
    {
        ...
        Assert.All(prices, p => Assert.Null(p.BatchInputPrice));
        Assert.All(prices, p => Assert.Null(p.BatchOutputPrice));
    }
    ...
    public void Normalize_SkipsIdsWithNoProviderSegment()
    {
        ...
        Assert.DoesNotContain(prices, p => p.ModelIdentifier == "no-provider-segment");
        Assert.DoesNotContain(prices, p => p.ModelIdentifier == "trailing-slash/");
    }
    ...
    public void Normalize_SkipsEntriesWithNoStandardRateOrNoPricingObject()
    {
        ...
        Assert.DoesNotContain(prices, p => p.ModelIdentifier == "some/moderation-only-model");
        Assert.DoesNotContain(prices, p => p.ModelIdentifier == "some/no-pricing-object");
    }
    ...
    public void Normalize_NonNumericPricingString_IsSkippedNotCoercedToZero()
    {
        ...
        Assert.Equal(10.0m, price.StandardOutputPrice);
    }
    ...
    private static OpenRouterPriceSourceClient Client() =>
        new(new HttpClient(), OpenRouterPriceSourceClient.DefaultUrl, NullLogger<OpenRouterPriceSourceClient>.Instance);
}
