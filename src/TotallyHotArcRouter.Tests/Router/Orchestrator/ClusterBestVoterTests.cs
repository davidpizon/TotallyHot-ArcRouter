...
public class ClusterBestVoterTests
{
    ...
    {
        var voter = CreateVoter(WriteArtifact(TwoClusterArtifact), new FakeMemoryEntryStore());
        var context = new VotingContext("live:bug_fixing", [new RoutingCandidate("model-a", "openai", IsFree: false)]);
    ...
    {
        var missingPath = Path.Combine(Path.GetTempPath(), "arcrouter-tests", Guid.NewGuid().ToString("N"), "cluster_model.json");
        var voter = CreateVoter(missingPath, new FakeMemoryEntryStore());
    ...
    public async Task VoteAsync_EmbeddingBelowAssignmentThreshold_AbstainsAsUnclustered()
    {
        ...
        store.Add([1f, 0f], "model-a", 1.0, count: 5);
        ...
        var voterWithHighThreshold = CreateVoter(
            WriteArtifact(TwoClusterArtifact), store, assignmentThreshold: 0.99);

        var vote = await voterWithHighThreshold.VoteAsync(context, TestContext.Current.CancellationToken);
    ...
    {
        var store = new FakeMemoryEntryStore();
        store.Add([1f, 0f], "model-a", 1.0, count: 1); // one observation, floor is 3
        var voter = CreateVoter(WriteArtifact(TwoClusterArtifact), store, minObservations: 3);
        var context = new VotingContext(
            "live:bug_fixing",
            [new RoutingCandidate("model-a", "openai", IsFree: false)],
            TaskEmbedding: [1f, 0f]);
    ...
    {
        var store = new FakeMemoryEntryStore();
        store.Add([1f, 0f], "model-a", 0.2, count: 3);
        store.Add([1f, 0f], "model-b", 0.9, count: 3);
        var voter = CreateVoter(WriteArtifact(TwoClusterArtifact), store, minObservations: 3);
        var context = new VotingContext(
            "live:bug_fixing",
    ...
    public async Task VoteAsync_ArtifactWrittenAfterConstruction_IsPickedUpOnlyAfterReload()
    {
        ...
        var path = Path.Combine(directory, "cluster_model.json");
        var store = new FakeMemoryEntryStore();
        store.Add([1f, 0f], "model-a", 1.0, count: 3);
        var voter = CreateVoter(path, store, minObservations: 3);
        var context = new VotingContext(
            "live:bug_fixing",
            [new RoutingCandidate("model-a", "openai", IsFree: false)],
            TaskEmbedding: [1f, 0f]);
        ...
        File.WriteAllText(path, ClusterModelArtifactSerializer.Serialize(TwoClusterArtifact));
        ...
        Assert.Equal("model-a", voteAfterReload.ModelName);
    ...
    public void Name_IsClusterBest()
    {
        ...
        Assert.Equal(VoterNames.ClusterBest, voter.Name);
    }
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "arcrouter-tests", Guid.NewGuid().ToString("N"), "cluster_model.json");
    ...
    private sealed class FakeMemoryEntryStore : IMemoryEntryStore
    {
        ...
        public void Add(float[] embedding, string model, double score, int count)
        {
            ...
            {
                _entries.Add(new MemoryEntry(_nextId++, embedding, model, score, Cost: 0.01, VerifierTrace: null, DateTimeOffset.UtcNow));
            }
        }
