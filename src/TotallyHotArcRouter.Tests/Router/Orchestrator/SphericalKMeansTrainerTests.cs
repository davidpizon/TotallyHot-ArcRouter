...
public class SphericalKMeansTrainerTests
{
    ...
    public void Train_TwoWellSeparatedClusters_RecoversThePlantedStructureDeterministically()
    {
        ...
        for (var i = 0; i < 20; i++) embeddings.Add(Jitter(baseVector: [1, 0, 0], seedOffset: i));
        for (var i = 0; i < 20; i++) embeddings.Add(Jitter(baseVector: [0, 1, 0], seedOffset: i));

        var result = SphericalKMeansTrainer.Train(embeddings: embeddings, minK: 2, maxK: 2, seed: 42);

        Assert.Equal(2, actual: result.ChosenK);
        Assert.Equal(2, actual: result.Centroids.Count);
        ...
        Assert.NotEqual(expected: firstGroupAssignments[0], actual: secondGroupAssignments[0]);
    }
    ...
    public void Train_SameInputsAndSeed_ProducesIdenticalAssignments()
    {
        ...
        for (var i = 0; i < 12; i++)
        {
            embeddings.Add(Jitter(baseVector: [1, 0, 0], seedOffset: i));
            embeddings.Add(Jitter(baseVector: [0, 1, 0], seedOffset: i));
            embeddings.Add(Jitter(baseVector: [0, 0, 1], seedOffset: i));
        }

        var first = SphericalKMeansTrainer.Train(embeddings: embeddings, minK: 3, maxK: 3, seed: 7);
        var second = SphericalKMeansTrainer.Train(embeddings: embeddings, minK: 3, maxK: 3, seed: 7);

        Assert.Equal(expected: first.Assignments, actual: second.Assignments);
        Assert.Equal(expected: first.KSelectionProvenance, actual: second.KSelectionProvenance);
    }
    ...
    public void Train_KSweepAcrossThreePlantedClusters_SilhouettePrefersThreeOverTwoOrFive()
    {
        ...
        var result = SphericalKMeansTrainer.Train(embeddings: embeddings, minK: 2, maxK: 5, seed: 42);

        Assert.Equal(3, actual: result.ChosenK);
        Assert.Contains(expectedSubstring: "Swept k in [2, 5]", actualString: result.KSelectionProvenance);
    }
    ...
    public void Train_KAboveSampleCount_CapsTheSweepInsteadOfThrowing()
    {
        ...
        var result = SphericalKMeansTrainer.Train(embeddings: embeddings, minK: 2, maxK: 100, seed: 1);
    ...
    public void Train_MismatchedEmbeddingLength_Throws()
    {
        ...
        Assert.Throws<ArgumentException>(() => SphericalKMeansTrainer.Train(embeddings: embeddings, minK: 1, maxK: 1));
    }
    ...
    public void Train_WeightsCountMismatch_Throws()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            SphericalKMeansTrainer.Train(embeddings: embeddings, weights: [1.0], 1, 1));
    }
    ...
    public void Train_HeavilyWeightedGroup_PullsItsCentroidTowardTheHeavyPoints()
    {
        ...
        var result = SphericalKMeansTrainer.Train(embeddings: embeddings, weights: weights, 1, 1, 1);
    ...
    /// <summary>
    /// Perturbs a unit vector slightly (deterministically, via <paramref name="seedOffset"/>) and re-normalizes, so a
    /// "cluster" of jittered copies is tight but not degenerately identical.
    /// </summary>
    private static float[] Jitter(float[] baseVector, int seedOffset)
    {
        ...
        for (var i = 0; i < jittered.Length; i++) jittered[i] /= magnitude;
