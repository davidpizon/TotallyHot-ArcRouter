...
/// <summary>
/// Covers <see cref="SqliteGraderScoreStore"/> over a real (temp-file) <see cref="RouterMemoryDatabase"/>,
/// mirroring <see cref="SqliteJudgeShadowScoreStoreTests"/>'s shape.
/// </summary>
public class SqliteGraderScoreStoreTests : IDisposable
{
    private readonly RouterMemoryDatabase _database;
    private readonly string _tempDirectory;

    public SqliteGraderScoreStoreTests()
    {
    ...
    {
        var store = new SqliteGraderScoreStore(_database);
    ...
    [Fact]
    public async Task GetAllAsync_RoundTripsEveryColumn_IncludingNullBackboneAndLength()
    {
        var store = new SqliteGraderScoreStore(_database);
        var withBackboneAndLength = MakeRecord("corr-1") with
        {
            GraderBackboneModel = "free-model", ResponseLengthChars = 512
        };
        var withoutEither = MakeRecord("corr-2") with { GraderBackboneModel = null, ResponseLengthChars = null };
        await store.InsertAsync(record: withBackboneAndLength, cancellationToken: TestContext.Current.CancellationToken);
        await store.InsertAsync(record: withoutEither, cancellationToken: TestContext.Current.CancellationToken);

        var rows = await store.GetAllAsync(TestContext.Current.CancellationToken);

        Assert.Equal(2, actual: rows.Count);
        var first = rows.Single(r => r.CorrelationId == "corr-1");
        Assert.Equal(expected: "free-model", actual: first.GraderBackboneModel);
        Assert.Equal(512, actual: first.ResponseLengthChars);
        var second = rows.Single(r => r.CorrelationId == "corr-2");
        Assert.Null(second.GraderBackboneModel);
        Assert.Null(second.ResponseLengthChars);
    }

    private static GraderScoreRecord MakeRecord(string correlationId)
    {
        return new GraderScoreRecord(
            ...
            GraderKey: "judge",
            Score: 0.7,
            GraderBackboneModel: "local-judge-model",
