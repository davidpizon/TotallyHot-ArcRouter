...
public class QualityGradingServiceTests
{
    ...
    public async Task ExecuteAsync_DrainsQueueAndSubmits()
    {
        ...
        var service = new QualityGradingService(
            ...
            StubGrader(),
            aggregator,
            Options.Create(new QualityOptions { Enabled = true, WorkerConcurrency = 1 }),
            NullLogger<QualityGradingService>.Instance);

        await service.StartAsync(TestContext.Current.CancellationToken);
        await aggregator.Completed.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        await service.StopAsync(TestContext.Current.CancellationToken);

        Assert.Equal(2, aggregator.Count);
    }
    ...
    public async Task ExecuteAsync_Disabled_DoesNotSubmit()
    {
        ...
        var service = new QualityGradingService(
            ...
            StubGrader(),
            aggregator,
            Options.Create(new QualityOptions { Enabled = false }),
            NullLogger<QualityGradingService>.Instance);
