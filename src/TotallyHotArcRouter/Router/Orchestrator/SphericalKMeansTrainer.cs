...
public static class SphericalKMeansTrainer
{
    ...
    /// <param name="KSelectionProvenance">A human-readable record of every <c>k</c> tried and its score, and which won - stamped onto the trained artifact so the choice is always explainable.</param>
    ...
    /// <param name="weights">Per-sample weights for centroid averaging, parallel to <paramref name="embeddings"/>. All-1.0 if omitted.</param>
    ...
    /// <exception cref="ArgumentException"><paramref name="embeddings"/> is empty, a vector length disagrees with the first, or <paramref name="minK"/>/<paramref name="maxK"/> are inconsistent.</exception>
        ...
        int maxIterations = 50)
    {
        ...
        ArgumentOutOfRangeException.ThrowIfLessThan(minK, 1);
        ArgumentOutOfRangeException.ThrowIfLessThan(maxK, minK);
        ...
        if (weights is not null && weights.Count != embeddings.Count)
        {
            throw new ArgumentException(
                $"weights has {weights.Count} entr(y/ies), expected {embeddings.Count} to match embeddings.",
                nameof(weights));
        }
        ...
        var effectiveWeights = weights ?? Enumerable.Repeat(1.0, embeddings.Count).ToArray();
        ...
        var effectiveMaxK = Math.Min(maxK, embeddings.Count);
        var effectiveMinK = Math.Min(minK, effectiveMaxK);
        ...
        {
            var (centroids, assignments) = RunOnce(unitEmbeddings, effectiveWeights, k, seed, maxIterations);
            var silhouette = ComputeApproximateSilhouette(unitEmbeddings, centroids, assignments);
            sweepLog.Add($"k={k}:{silhouette:F4}");

            if (best is null || silhouette > best.SilhouetteScore)
            {
                best = new TrainResult(centroids, assignments, k, silhouette, string.Empty);
            }
        }

        var provenance =
            $"Swept k in [{effectiveMinK}, {effectiveMaxK}] under seed {seed}; chose k={best!.ChosenK} " +
            $"(silhouette={best.SilhouetteScore:F4}). Per-k silhouette: {string.Join(", ", sweepLog)}.";
        ...
        float[][] embeddings, IReadOnlyList<double> weights, int k, int seed, int maxIterations)
    {
        ...
        var centroids = InitializeKMeansPlusPlus(embeddings, k, random);
        ...
        for (var iteration = 0; iteration < maxIterations; iteration++)
        {
            ...
            {
                var nearest = NearestCentroid(embeddings[i], centroids);
            ...
            var recomputed = RecomputeCentroids(embeddings, weights, assignments, centroids, random);
    ...
    private static float[][] InitializeKMeansPlusPlus(float[][] embeddings, int k, Random random)
    {
        ...
        while (centroids.Count < k)
        {
            ...
            {
                var nearest = NearestCentroid(embeddings[i], [.. centroids]);
                var distance = 1.0 - Dot(embeddings[i], centroids[nearest]);
    ...
    private static float[][] RecomputeCentroids(
        float[][] embeddings, IReadOnlyList<double> weights, int[] assignments, float[][] previousCentroids, Random random)
    {
        var dimension = embeddings[0].Length;
        ...
        for (var i = 0; i < embeddings.Length; i++)
        {
            ...
            {
                sums[cluster][d] += weight * embeddings[i][d];
            }
        }
        ...
        for (var c = 0; c < k; c++)
        {
            ...
            for (var d = 0; d < dimension; d++)
            {
                vector[d] = (float)(sums[c][d] / totalWeights[c]);
            }
    ...
    /// <summary>Returns the index of the centroid with the highest cosine similarity (dot product on unit vectors) to <paramref name="embedding"/>.</summary>
    private static int NearestCentroid(float[] embedding, float[][] centroids)
    {
        ...
        {
            var similarity = Dot(embedding, centroids[c]);
    ...
    private static double ComputeApproximateSilhouette(float[][] embeddings, float[][] centroids, int[] assignments)
    {
        ...
        for (var i = 0; i < embeddings.Length; i++)
        {
            ...
            var a = 1.0 - Dot(embeddings[i], centroids[own]);
            ...
            for (var c = 0; c < centroids.Length; c++)
            {
                ...
                var distance = 1.0 - Dot(embeddings[i], centroids[c]);
                if (distance < b)
                {
                    b = distance;
                }
    ...
    /// <summary>Computes the dot product of two equal-length vectors, which is the cosine similarity for unit-normalized inputs.</summary>
