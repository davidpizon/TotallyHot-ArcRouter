...
public sealed class LlamaPayloadTranslator : IBedrockPayloadTranslator
{
    ...
    public byte[] TranslateRequest(byte[] openAiShapedBody)
    {
        ...
        return JsonSerializer.SerializeToUtf8Bytes(value: llama, options: SerializerOptions);
    }
    ...
    {
        return new LlamaStreamChunkTranslator(Provider);
    }
    ...
    private static string BuildPrompt(JsonArray messages)
    {
        ...
        if (lastTemplateRole != "assistant") builder.Append("<|start_header_id|>assistant<|end_header_id|>\n\n");

        return builder.ToString();
    }

    /// <summary>Maps Llama's <c>stop_reason</c> ("stop"/"length", per AWS's docs) to OpenAI's <c>finish_reason</c>.</summary>
    internal static string MapStopReason(string? reason)
    {
...
/// <summary>
/// Translates each of Llama's already-decoded streaming chunks (
/// <c>
/// {generation, prompt_token_count?,
/// generation_token_count?, stop_reason?}
/// </c>
/// ) into an OpenAI-shaped SSE chunk. <c>stop_reason</c> is
...
internal sealed class LlamaStreamChunkTranslator : IBedrockStreamChunkTranslator
{
    ...
    /// The stable model identifier to report (the provider key, since Llama's own chunks carry
    /// none).
    /// </param>
    public LlamaStreamChunkTranslator(string modelIdentifier)
    {
    ...
    public byte[] TranslateChunk(byte[] nativeChunkJson)
    {
        ...
        if (generation.Length > 0) delta["content"] = generation;
