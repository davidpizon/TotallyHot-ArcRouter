...
public sealed class TitanPayloadTranslator : IBedrockPayloadTranslator
{
    ...
    public byte[] TranslateRequest(byte[] openAiShapedBody)
    {
        ...
        return JsonSerializer.SerializeToUtf8Bytes(titan, SerializerOptions);
    }
        ...
        foreach (var messageNode in messages)
        {
            ...
            switch (message["role"]?.GetValue<string>())
            {
                ...
                case "function":
                    ...
                    if (text.Length > 0)
                    {
                        builder.Append("Tool: ").Append(text).Append('\n');
                    }
    ...
    internal static string MapCompletionReason(string? reason) => reason switch
    {
        null or "" => "stop",
        "FINISHED" => "stop",
    ...
    /// <inheritdoc />
    public IBedrockStreamChunkTranslator CreateBedrockStreamChunkTranslator() => new TitanStreamChunkTranslator(Provider);
}

/// <summary>
/// Translates each of Titan's already-decoded streaming chunks (<c>{index, inputTextTokenCount,
/// totalOutputTextTokenCount, outputText, completionReason}</c>, per AWS's documented shape) into an
...
internal sealed class TitanStreamChunkTranslator : IBedrockStreamChunkTranslator
{
    ...
    public byte[] TranslateChunk(byte[] nativeChunkJson)
    {
        ...
        var finishReason = string.IsNullOrEmpty(completionReason) ? null : TitanPayloadTranslator.MapCompletionReason(completionReason);
