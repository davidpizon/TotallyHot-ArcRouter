...
public sealed class GeminiPayloadTranslator : IPayloadTranslator
{
    ...
    public byte[] TranslateRequest(byte[] openAiShapedBody)
    {
        ...
        {
            gemini["system_instruction"] = systemInstruction;
        }
        ...
        if (TranslateToolChoice(root["tool_choice"]) is { } toolConfig)
        {
            gemini["toolConfig"] = toolConfig;
        }

        if (TranslateGenerationConfig(root) is { } generationConfig)
        {
            gemini["generationConfig"] = generationConfig;
        }

        return JsonSerializer.SerializeToUtf8Bytes(gemini, SerializerOptions);
    }
    ...
    private static (JsonObject? SystemInstruction, JsonArray Contents) TranslateMessages(JsonArray messages)
    {
        ...
        foreach (var messageNode in messages)
        {
            ...
            switch (role)
            {
                ...
                case "assistant":
                    AppendAssistantContent(contents, message);
                    break;

                case "tool":
                case "function":
                    AppendToolResult(contents, message);
                    break;
        ...
        JsonObject? systemInstruction = systemParts.Count > 0
        ...
        if (contents.Count == 0)
        {
            AppendTextContent(contents, "user", " ");
        }

        return (systemInstruction, contents);
    }

    /// <summary>Builds a "model" turn's Gemini parts from an assistant message's text and OpenAI-style tool_calls, converted into functionCall parts.</summary>
    private static void AppendAssistantContent(JsonArray contents, JsonObject message)
    {
        ...
        AppendMergedContent(contents, "model", parts);
    }

    /// <summary>Appends a tool result as a Gemini functionResponse part on a user turn, wrapping a plain-string result under a conventional "content" key when it isn't already valid JSON.</summary>
    private static void AppendToolResult(JsonArray contents, JsonObject message)
    {
        ...
        AppendMergedContent(contents, "user", new JsonArray { part });
    }

    /// <summary>Appends a single text part for the given role, substituting a blank space when the text is empty since Gemini rejects a turn with no text part.</summary>
    private static void AppendTextContent(JsonArray contents, string role, string? text)
    {
        ...
        AppendMergedContent(contents, role, parts);
    }

    /// <summary>Appends parts to the last content when it shares <paramref name="role"/>, else starts a new content - the consecutive-role merge Gemini requires.</summary>
    ...
    /// <summary>Maps OpenAI <c>tools</c> to Gemini's single <c>{ functionDeclarations: [...] }</c> tool object; returns null when there are no function tools.</summary>
    ...
    /// <summary>Maps OpenAI <c>tool_choice</c> to Gemini's <c>functionCallingConfig.mode</c>, treating any object form (naming a specific function) as ANY; returns null when there is no equivalent mode.</summary>
    ...
    /// <summary>Maps OpenAI's sampling and output-shaping request fields (temperature, top_p, top_k, max tokens, n, stop, response_format) to Gemini's <c>generationConfig</c> object, returning null when none of those fields were present.</summary>
    private static JsonObject? TranslateGenerationConfig(JsonObject root)
    {
        ...
        if (root["n"] is JsonNode candidateCount)
        {
            config["candidateCount"] = candidateCount.DeepClone();
        }
    ...
    public byte[] TranslateResponse(byte[] nativeShapedBody)
    {
        ...
        if (root["usageMetadata"] is JsonObject usageMetadata)
        {
            openAi["usage"] = TranslateUsage(usageMetadata);
        }

        return JsonSerializer.SerializeToUtf8Bytes(openAi, SerializerOptions);
    }

    /// <summary>Translates one Gemini candidate into an OpenAI choice, concatenating text parts, converting functionCall parts to tool_calls, and mapping the finish reason (always "tool_calls" when any tool call is present).</summary>
    private static JsonObject TranslateCandidate(JsonObject candidate)
    {
        ...
        if (candidate["content"] is JsonObject content && content["parts"] is JsonArray parts)
        {
            foreach (var partNode in parts)
            {
                ...
                {
                    toolCalls.Add(BuildToolCall(functionCall, toolCalls.Count));
                }
            }
        }
    ...
    /// <summary>Converts a Gemini functionCall part into an OpenAI-shaped tool_calls entry, generating a synthetic id and serializing the args object to a JSON string for the arguments field.</summary>
    ...
    /// <summary>Maps Gemini's <c>usageMetadata</c> token counts to OpenAI's <c>prompt_tokens</c>/<c>completion_tokens</c>/<c>total_tokens</c> shape, falling back to a computed total when Gemini omits <c>totalTokenCount</c>.</summary>
    ...
    internal static string MapFinishReason(string? geminiFinishReason) => geminiFinishReason switch
    {
        null or "" => "stop",
        "STOP" => "stop",
        "MAX_TOKENS" => "length",
    ...
    /// <inheritdoc />
    public IStreamTranslator CreateStreamTranslator() => new GeminiStreamTranslator();
    ...
    internal static bool TryExtractEmbeddedError(byte[] body, out string status, out string message)
    {
        ...
        var jsonText = text.StartsWith("data:", StringComparison.Ordinal) ? text[5..].Trim() : text;
