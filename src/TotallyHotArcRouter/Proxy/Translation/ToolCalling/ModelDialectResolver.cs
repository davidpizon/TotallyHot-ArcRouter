...
///
/// <para>
/// <b>An unresolved model writes no row at all.</b> That is the deliberate design, not a gap: a missing row
/// means "forward natively and classify from the first real response", which is both correct and free,
/// whereas a wrong row means arming the wrong scanner against every response the model produces. Silence
/// is strictly better than a guess here.
/// </para>
/// </summary>
/// <remarks>
...
public sealed class ModelDialectResolver
{
    ...
    /// <returns>
    /// The detected capability, or <see langword="null"/> when no tier could answer - see the type-level
    /// note on why that is recorded as nothing rather than as a guess.
    /// </returns>
    ...
    public async Task<ModelToolCapability?> ResolveAsync(
        string providerKey,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var template = await TryReadOllamaTemplateAsync(provider, upstreamId, cancellationToken).ConfigureAwait(false);
            if (template is not null)
            {
                var matched = MatchTemplate(template);
                ...
                {
                    return Capability(providerKey, modelName, matched.Value.Dialect, DetectionConfidence.Template,
                        $"Ollama /api/show template contains '{matched.Value.Delimiter}'.");
                }
                ...
                return RendersTools(template)
                    ? null
                    : Capability(providerKey, modelName, ToolCallDialectRegistry.Emulated, DetectionConfidence.Template,
                        "Ollama /api/show template renders no tools.");
            }
        }
        ...
        {
            var architecture = await TryReadLmStudioArchitectureAsync(provider, upstreamId, cancellationToken).ConfigureAwait(false);
            if (architecture is not null && TryMapArchitecture(architecture, out var archDialect))
            {
                return Capability(providerKey, modelName, archDialect, DetectionConfidence.Template,
                    $"LM Studio reports architecture '{architecture}'.");
            }
    ...
    /// </remarks>
    private async Task<string?> TryReadOllamaTemplateAsync(
    ...
    /// <summary>
    /// Reads a model's architecture from LM Studio's native <c>GET /api/v0/models</c>, or returns
    /// <see langword="null"/> when the endpoint could not answer for it.
    /// </summary>
    private async Task<string?> TryReadLmStudioArchitectureAsync(
        ProviderOptions provider, string modelId, CancellationToken cancellationToken)
    {
        var root = ProviderUrlBuilder.StripVersionSuffix(provider.BaseUrl);
        ...
        try
        {
            ...
            foreach (var entry in data.EnumerateArray())
            {
                ...
                return entry.TryGetProperty("arch", out var arch) && arch.ValueKind == JsonValueKind.String
                    ? arch.GetString()
                    : null;
            }
