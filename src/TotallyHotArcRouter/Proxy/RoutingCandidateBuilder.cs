    ...
    ILogger logger)
{
    ...
    public List<(string ModelName, ResolvedModelRoute Route)> GetEligibleRoutes(
        IReadOnlyCollection<string> excludeModelNames)
    {
        var excluded = new HashSet<string>(collection: excludeModelNames, comparer: StringComparer.OrdinalIgnoreCase);
        var eligible = new List<(string ModelName, ResolvedModelRoute Route)>();

        foreach (var candidate in modelRouteResolver.ListModels())
        {
            if (excluded.Contains(candidate.ModelName)) continue;

            if (!modelRouteResolver.TryResolve(modelName: candidate.ModelName, route: out var candidateRoute)) continue;
...
/// <param name="Route">
/// The primary route actually used to build <paramref name="Candidates"/>[0] - the original route, or
/// its circuit-breaker/administrative substitute.
/// </param>
/// <param name="SubstitutionReason">
/// Why <paramref name="Route"/> differs from the route
/// <see cref="RoutingCandidateBuilder.Build"/> was called with, or the reason passed in when it doesn't.
/// </param>
