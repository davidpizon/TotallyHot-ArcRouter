...
public sealed class UsageRollupStore : IUsageRollupStore, IDisposable
{
    ...
    public void EnsureBucketTimezone()
    {
        ...
        {
            _logger?.LogWarning(exception: ex, message: "Failed to pin the usage-rollup bucket timezone.");
        }
    }
    ...
    public async Task<int> RollForwardAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var pending = ReadPendingEntries(connection: connection, checkpoint: checkpoint);
            if (pending.Count == 0) return 0;

            var tz = ResolveTimeZone(connection);
            foreach (var (_, entry) in pending) ApplyIncrement(connection: connection, entry: entry, tz: tz);

            WriteCheckpoint(connection: connection, entryId: pending[^1].EntryId);
        ...
        catch (Exception ex)
        {
            _logger?.LogWarning(exception: ex,
                message: "Usage-rollup roll-forward failed; will retry on the next write or startup.");
            return 0;
        }
        finally
        {
    ...
    /// <inheritdoc/>
    public IReadOnlyList<UsageRollupBucket> Query(DateTimeOffset from, DateTimeOffset to, string bucketWidth,
        string groupBy)
    {
    ...
    {
        var rows = ReadRawBuckets(from: from, to: to, bucketWidth: "P1D");
    ...
    /// <summary>
    /// Aggregates a group of raw rollup rows into a single <see cref="UsageRollupBucket"/> by summing every numeric
    /// column.
    /// </summary>
    /// <param name="bucketStartUtc">
    /// The bucket start to report on the result (the group's own key for a day grouping,
    /// otherwise the query's <c>from</c> bound).
    /// </param>
    ...
    private List<RawRow> ReadRawBuckets(DateTimeOffset from, DateTimeOffset to, string bucketWidth)
    {
        ...
        command.CommandText = """
                              ...
                              """;
        command.Parameters.AddWithValue(parameterName: "$width", value: bucketWidth);
        command.Parameters.AddWithValue(parameterName: "$from",
            value: from.UtcDateTime.ToString(format: TimestampFormat, provider: CultureInfo.InvariantCulture));
        command.Parameters.AddWithValue(parameterName: "$to",
            value: to.UtcDateTime.ToString(format: TimestampFormat, provider: CultureInfo.InvariantCulture));
        ...
        while (reader.Read())
        {
            ...
            if (BucketEndUtc(bucketStartUtc: bucketStart, tz: tz, width: bucketWidth) > now) continue;

            rows.Add(new RawRow(
                BucketStartUtc: bucketStart,
                Provider: reader.GetString(1),
                Model: reader.GetString(2),
                Requests: reader.GetInt64(3),
                UnpricedRequests: reader.GetInt64(4),
                PromptTokens: reader.GetInt64(5),
                CompletionTokens: reader.GetInt64(6),
                CacheCreationTokens: reader.GetInt64(7),
                CacheReadTokens: reader.GetInt64(8),
                CostUsd: decimal.Parse(s: reader.GetString(9), provider: CultureInfo.InvariantCulture)));
        }
    ...
    private static TimeSpan WidthToDuration(string width)
    {
        return width switch
        {
            ...
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(width), actualValue: width,
                message: "Unknown bucket width; expected 'PT30M', 'PT1H', or 'P1D'.")
        };
    }
    ...
    /// <summary>
    /// Computes the UTC instant a bucket that started at <paramref name="bucketStartUtc"/> ends, honoring DST
    /// transitions for day-width buckets.
    /// </summary>
    ...
    {
        if (width != "P1D") return bucketStartUtc + WidthToDuration(width);

        var localStart = TimeZoneInfo.ConvertTime(dateTimeOffset: bucketStartUtc, destinationTimeZone: tz).DateTime;
        var nextLocalMidnight = localStart.Date.AddDays(1);
        if (tz.IsInvalidTime(nextLocalMidnight)) nextLocalMidnight = nextLocalMidnight.AddHours(1);

        return TimeZoneInfo.ConvertTimeToUtc(dateTime: nextLocalMidnight, sourceTimeZone: tz);
    }
    ...
    // type remarks).
    /// <summary>
    /// Pins the configured rollup timezone into <c>rollup_metadata</c> if no row exists yet; a no-op once pinned,
    /// since the timezone is write-once.
    /// </summary>
    ...
    private void PinTimezoneIfMissing(SqliteConnection connection)
    {
        ...
        command.CommandText = """
                              ...
                              """;
        command.Parameters.AddWithValue(parameterName: "$tz", value: _configuredTimezoneId);
        command.Parameters.AddWithValue(parameterName: "$pinnedAt",
            value: DateTimeOffset.UtcNow.ToString(format: TimestampFormat,
    ...
    {
        if (_cachedTimeZone is { } cached) return cached;
        ...
        {
            _logger?.LogWarning(exception: ex,
                message: "Pinned rollup timezone {TimezoneId} could not be resolved on this host; falling back to UTC.",
                id);
            tz = TimeZoneInfo.Utc;
        }
    ...
    private static long ReadCheckpoint(SqliteConnection connection)
    {
        ...
        return result is null or DBNull ? 0 : Convert.ToInt64(value: result, provider: CultureInfo.InvariantCulture);
    }

    /// <summary>
    /// Advances the roll-forward checkpoint to <paramref name="entryId"/>, marking every entry up to and including it
    /// as applied.
    /// </summary>
    /// <param name="connection">The open database connection to write through.</param>
    ...
    private static void WriteCheckpoint(SqliteConnection connection, long entryId)
    {
        ...
        command.Parameters.AddWithValue(parameterName: "$entryId", value: entryId);
        command.ExecuteNonQuery();
    }

    /// <summary>
    /// Reads every <c>usage_ledger</c> entry with an id greater than <paramref name="checkpoint"/>, in ascending id
    /// order.
    /// </summary>
    /// <param name="connection">The open database connection to read from.</param>
    ...
    private static List<(long EntryId, UsageLedgerEntry Entry)> ReadPendingEntries(SqliteConnection connection,
        long checkpoint)
    {
        using var command = connection.CreateCommand();
        command.CommandText = """
                              ...
                              ORDER BY entry_id;
                              """;
        command.Parameters.AddWithValue(parameterName: "$checkpoint", value: checkpoint);
        ...
        while (reader.Read())
        {
            ...
            var entry = new UsageLedgerEntry(
                ...
                EstimatedCostUsd: reader.IsDBNull(10)
                    ? null
                    : decimal.Parse(s: reader.GetString(10), provider: CultureInfo.InvariantCulture),
    ...
    private void ApplyIncrement(SqliteConnection connection, UsageLedgerEntry entry, TimeZoneInfo tz)
    {
        ...
        foreach (var width in RollupWidths)
        {
            ...
            try
            {
                bucketStartUtc = BucketStartUtc(occurredAtUtc: entry.OccurredAtUtc, tz: tz, width: width);
            }
            catch (Exception ex)
            {
                _logger?.LogWarning(
                    exception: ex,
                    message: "Failed to compute a {Width} rollup bucket for session {SessionId}; skipping that bucket.",
                    width,
                    SanitizeForLog(entry.SessionId));
                continue;
            }

            UpsertBucket(connection: connection, bucketStartUtc: bucketStartUtc, bucketWidth: width,
                provider: entry.Provider, model: entry.ResolvedModel, unpriced: unpriced, promptTokens: prompt,
                completionTokens: completion, cacheCreationTokens: cacheCreation, cacheReadTokens: cacheRead,
                cost: cost);
        }
    }
    ...
    /// <summary>
    /// Floors <paramref name="occurredAtUtc"/> to a bucket boundary in the pinned wall-clock timezone, then converts
    /// that boundary back to UTC for storage.
    /// </summary>
    ...
    {
        var local = TimeZoneInfo.ConvertTime(dateTimeOffset: occurredAtUtc, destinationTimeZone: tz).DateTime;
        var floored = width switch
        {
            "PT30M" => new DateTime(year: local.Year, month: local.Month, day: local.Day, hour: local.Hour,
                minute: local.Minute - local.Minute % 30, 0, kind: DateTimeKind.Unspecified),
            "PT1H" => new DateTime(year: local.Year, month: local.Month, day: local.Day, hour: local.Hour, 0, 0,
                kind: DateTimeKind.Unspecified),
            "P1D" => new DateTime(year: local.Year, month: local.Month, day: local.Day, 0, 0, 0,
                kind: DateTimeKind.Unspecified),
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(width), actualValue: width,
                message: "Unknown bucket width; expected 'PT30M', 'PT1H', or 'P1D'.")
        };

        if (tz.IsInvalidTime(floored)) floored = floored.AddHours(1);

        return TimeZoneInfo.ConvertTimeToUtc(dateTime: floored, sourceTimeZone: tz);
    }
    ...
    private void UpsertBucket(
        ...
        decimal cost)
    {
        var bucketStartText = bucketStartUtc.ToString(format: TimestampFormat, provider: CultureInfo.InvariantCulture);
        ...
        using (var read = connection.CreateCommand())
        {
            ...
            read.CommandText = """
                               ...
                               """;
            read.Parameters.AddWithValue(parameterName: "$b", value: bucketStartText);
            read.Parameters.AddWithValue(parameterName: "$w", value: bucketWidth);
            read.Parameters.AddWithValue(parameterName: "$p", value: provider);
            read.Parameters.AddWithValue(parameterName: "$m", value: model);
        ...
        using (var upsert = connection.CreateCommand())
        {
            ...
            upsert.CommandText = """
                                 ...
                                 """;
            upsert.Parameters.AddWithValue(parameterName: "$b", value: bucketStartText);
            upsert.Parameters.AddWithValue(parameterName: "$w", value: bucketWidth);
            upsert.Parameters.AddWithValue(parameterName: "$p", value: provider);
            upsert.Parameters.AddWithValue(parameterName: "$m", value: model);
            upsert.Parameters.AddWithValue(parameterName: "$unpriced", value: unpriced);
            ...
            upsert.Parameters.AddWithValue(parameterName: "$cost",
                value: (existingCost + cost).ToString(CultureInfo.InvariantCulture));
