...
public sealed class RateLimitHeaderCapture : IRateLimitHeaderCapture, IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (!Array.Exists(array: HeaderPrefixes,
                    match: prefix =>
                        header.Key.StartsWith(value: prefix, comparisonType: StringComparison.OrdinalIgnoreCase)))
                continue;

            var value = string.Join(separator: ", ", values: header.Value);
            matched.Add(new RateLimitHeaderRow(HeaderName: header.Key.ToLowerInvariant(), HeaderValue: value));
        }
        ...
        if (!_channel.Writer.TryWrite(new CaptureItem(ProviderKey: providerKey, Headers: matched,
                ObservedAtUtc: DateTimeOffset.UtcNow, Completion: completion)))
        {
            ...
            _logger?.LogDebug(
                message: "Rate-limit header capture queue is full or closed; dropping capture for provider {Provider}.",
    ...
    private async Task ConsumeAsync()
    {
        await foreach (var item in _channel.Reader.ReadAllAsync().ConfigureAwait(false))
            try
            {
                _repository.UpsertRateLimitHeaders(providerKey: item.ProviderKey, headers: item.Headers,
            ...
            catch (Exception ex)
            {
                ...
                _logger?.LogWarning(exception: ex,
                    message: "Failed to capture rate-limit headers for provider {Provider}.",
                    SanitizeForLog(item.ProviderKey));
            }
            finally
            {
