...
public static class PublishedChecksumHasher
{
    ...
    public static string Compute(Stream content, long length, PublishedChecksumAlgorithm algorithm,
        CancellationToken cancellationToken = default)
    {
        return algorithm switch
        {
            PublishedChecksumAlgorithm.GitBlobSha1 => GitBlobHash.Compute(content: content, length: length,
                cancellationToken: cancellationToken),
            PublishedChecksumAlgorithm.LfsSha256 => ContentSha256Hash.Compute(content: content,
                cancellationToken: cancellationToken),
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(algorithm), actualValue: algorithm,
                message: "Unrecognized published checksum algorithm.")
        };
    }
}