...
public sealed class UpdateCheckHostedService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            await Task.Delay(delay: InitialDelayOverride, cancellationToken: stoppingToken).ConfigureAwait(false);
        }
    ...
    internal async Task RunOneCheckAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (result.IsUpdateAvailable)
                ...
                _logger.LogInformation(
                    message: "Update check could not resolve: {Reason} ({Detail})",
                    result.UnavailableReason,
                    result.UnavailableDetail);
            else
                _logger.LogDebug(message: "Update check: Router is up to date at {CurrentVersion}.",
                    result.CurrentVersion);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            _logger.LogError(exception: ex, message: "Update check threw unexpectedly; continuing.");
        }
    }
}