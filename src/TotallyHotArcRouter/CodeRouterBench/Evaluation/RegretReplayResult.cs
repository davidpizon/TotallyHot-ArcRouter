...
public sealed class RegretReplayResult
{
    ...
    /// </summary>
    public double CumulativeRegret { get; private set; }

    /// <summary>Gets <c>AvgPerf</c> — the mean verifier score <c>s_i(a_i)</c> of the model actually selected.</summary>
    public double AvgPerf => ScoredTaskCount == 0 ? 0d : _totalScore / ScoredTaskCount;
    ...
    public int ScoredTaskCount { get; private set; }
    ...
    /// </summary>
    /// <param name="outcome">
    /// The task's full outcome row (every model the corpus scored, independent of what the router could
    /// see).
    /// </param>
    ...
    public void Record(RegretTaskOutcome outcome, string? selectedModelId, RewardWeights weights)
    {
        ...
        if (outcome.Cells.Count == 0)
            throw new ArgumentException(
                message: "A task outcome must have at least one cell to compute an oracle from.",
                paramName: nameof(outcome));
        ...
        if (!outcome.Cells.TryGetValue(key: selectedModelId, value: out var selectedCell))
            throw new ArgumentException(
                message:
                ...
                paramName: nameof(selectedModelId));

        var oracleReward = outcome.Cells.Values.Max(weights.Reward);
        var selectedReward = weights.Reward(selectedCell);

        CumulativeRegret += oracleReward - selectedReward;
        _totalScore += selectedCell.Score;
        TotalCostUsd += selectedCell.CostUsd;
        TotalTokens += selectedCell.TotalTokens ?? 0L;
        ScoredTaskCount++;
    }
}