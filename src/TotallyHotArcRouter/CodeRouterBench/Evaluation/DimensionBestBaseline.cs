...
public sealed class DimensionBestBaseline : IRegretBaselineRouter
{
    ...
    /// <param name="matrix">The frozen probing-split score matrix, e.g. from <see cref="DimensionModelScoreMatrix.FromDatabase"/>.</param>
    ...
    public string? Route(RegretReplayContext context)
    {
        ...
        return context.CandidateModelIds
            .Select(id => (Model: id, Score: _matrix.AverageScore(context.Dimension, id)))
