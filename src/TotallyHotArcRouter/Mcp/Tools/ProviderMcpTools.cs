...
public sealed class ProviderMcpTools
{
    ...
    [Description(
        "Lists every configured provider with its models, budget, and current-month spend. Credentials are masked: a custom header's value is returned only when that header is unlocked; a locked header reports its source alone.")]
    public ProvidersResponse ListProviders()
    {
        return _facade.ListProviders();
    }

    /// <summary>Adds or edits a provider.</summary>
    [McpServerTool(Name = "upsert_provider")]
    [Description(
        "Adds a new provider or edits an existing one. Authentication is expressed as an ordinary entry in Headers (e.g. an 'Authorization' or 'x-api-key' header); a header's value is write-only and never echoed back by any tool. A blank literal header value preserves the currently stored value; set Locked to withhold it from future reads.")]
    public async Task<object> UpsertProviderAsync(
        [Description("The provider key, e.g. 'openai'.")]
        string key,
        [Description(
            "The provider fields to write; non-credential fields fall back to the existing value when omitted.")]
        ProviderWriteRequest request,
        CancellationToken cancellationToken)
    {
        var result = await _facade.UpsertProviderAsync(key: key, request: request, cancellationToken: cancellationToken)
    ...
    [Description(
        "Removes a provider by key, along with every model route that points at it. Rejected if the provider is unknown. Historical spend and usage metrics are retained.")]
    public async Task<object> RemoveProviderAsync(
        [Description("The provider key to remove.")]
        string key,
        CancellationToken cancellationToken)
    {
        var result = await _facade.RemoveProviderAsync(key: key, cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        return ToToolResult(result);
    }
    ...
    public async Task<object> UpsertModelAsync(
        ...
        string modelName,
        [Description("The upstream model identifier; defaults to modelName when blank.")]
        ModelWriteRequest request,
        CancellationToken cancellationToken)
    {
        var result = await _facade.UpsertModelAsync(providerKey: providerKey, modelName: modelName, request: request,
    ...
    public async Task<object> RemoveModelAsync(
        [Description("The client-facing model name to remove.")]
        string modelName,
        CancellationToken cancellationToken)
    {
        var result = await _facade.RemoveModelAsync(modelName: modelName, cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        return ToToolResult(result);
    }
    ...
    [Description(
        "Sets or clears a provider's monthly USD and/or token budget cap. A null cap clears that dimension; both null removes the budget.")]
    ...
    {
        return ToToolResult(_facade.SetBudget(providerKey: providerKey, request: request));
    }
    ...
    public async Task<object> DiscoverModelsAsync(
        [Description("The provider key to query.")]
        string providerKey,
        CancellationToken cancellationToken)
    {
        var result = await _facade.DiscoverModelsAsync(providerKey: providerKey, cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        return ToToolResult(result);
    }
    ...
    [Description(
        "Probes which API flavors a provider's endpoint answers (OpenAI-compatible, LM Studio native, Ollama native, Anthropic-compatible) and records the result.")]
    public async Task<object> ScanProviderCapabilitiesAsync(
        [Description("The provider key to scan.")]
        string providerKey,
        CancellationToken cancellationToken)
    {
        var result = await _facade.ScanCapabilitiesAsync(key: providerKey, cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        return ToToolResult(result);
    }

    /// <summary>
    /// Maps a facade <see cref="ManagementResult{T}"/> to either the success value or a small error-shaped object (
    /// <c>{ error, type }</c>).
    /// </summary>
