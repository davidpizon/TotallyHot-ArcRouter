...
/// The lowercase header name (e.g. <c>anthropic-ratelimit-input-tokens-remaining</c> or
/// <c>x-ratelimit-remaining-tokens</c>).
/// </param>
...
/// </summary>
/// <param name="Limit">
/// The dimension's configured cap, or <see langword="null"/> if the <c>-limit</c> header was
/// absent/unparsable.
/// </param>
/// <param name="Remaining">
/// What's left before the cap, or <see langword="null"/> if the <c>-remaining</c> header was
/// absent/unparsable.
/// </param>
/// <param name="ResetAt">
/// When the dimension resets, or <see langword="null"/> if the <c>-reset</c> header was
/// absent/unparsable.
/// </param>
...
/// capture.
/// </param>
/// <param name="UnifiedStatus">
/// The unified family's top-level <c>anthropic-ratelimit-unified-status</c>, or
/// <see langword="null"/> if absent.
/// </param>
/// <param name="UnifiedResetAt">
/// The unified family's top-level <c>anthropic-ratelimit-unified-reset</c>, or
/// <see langword="null"/> if absent/unparsable.
/// </param>
...
/// </param>
/// <param name="RepresentativeClaim">
/// The unified family's <c>anthropic-ratelimit-unified-representative-claim</c>, or
/// <see langword="null"/> if absent.
/// </param>
/// <param name="RawHeaders">
/// Every captured header, verbatim, keyed by lowercase name - the fallback for anything this
/// parser doesn't specifically recognize.
/// </param>
...
public static class RateLimitSnapshotParser
{
    ...
    private static readonly Regex GoDurationTokenPattern =
        new(pattern: @"(\d+(?:\.\d+)?)(ms|h|m|s)", options: RegexOptions.Compiled);
        ...
        DateTimeOffset? observedAtUtc = null)
    {
        ...
        foreach (var row in rows) raw[row.HeaderName] = row.HeaderValue;
        ...
        foreach (var dimension in StandardDimensionNames)
        {
            ...
            if (!raw.ContainsKey(limitHeader) && !raw.ContainsKey(remainingHeader) &&
                !raw.ContainsKey(resetHeader)) continue;

            standard[dimension] = new RateLimitDimensionView(
                Limit: ReadLong(raw: raw, headerName: limitHeader),
                Remaining: ReadLong(raw: raw, headerName: remainingHeader),
                ResetAt: ReadDate(raw: raw, headerName: resetHeader));
        }
        ...
        foreach (var dimension in OpenAiDimensionNames)
        {
            ...
            standard[key] = new RateLimitDimensionView(
                Limit: ReadLong(raw: raw, headerName: limitHeader),
                Remaining: ReadLong(raw: raw, headerName: remainingHeader),
                ResetAt: ReadGoDurationResetDate(raw: raw, headerName: resetHeader, observedAtUtc: observedAtUtc));
        }

        var unifiedStatus = ReadString(raw: raw, headerName: $"{UnifiedPrefix}status");
        var unifiedResetAt = ReadDate(raw: raw, headerName: $"{UnifiedPrefix}reset");
        var representativeClaim = ReadString(raw: raw, headerName: $"{UnifiedPrefix}representative-claim");
        ...
        {
            if (!headerName.StartsWith(value: UnifiedPrefix, comparisonType: StringComparison.OrdinalIgnoreCase))
                continue;

            var suffix = headerName[UnifiedPrefix.Length..];
            if (string.Equals(a: suffix, b: "status", comparisonType: StringComparison.OrdinalIgnoreCase) ||
                string.Equals(a: suffix, b: "reset", comparisonType: StringComparison.OrdinalIgnoreCase) ||
                string.Equals(a: suffix, b: "representative-claim", comparisonType: StringComparison.OrdinalIgnoreCase))
                // Already handled as top-level unified fields above, not a per-window one.
                continue;

            var dashIndex = suffix.LastIndexOf('-');
            if (dashIndex <= 0 || dashIndex == suffix.Length - 1) continue;

            var windowName = suffix[..dashIndex];
            var field = suffix[(dashIndex + 1)..];

            windows.TryGetValue(key: windowName, value: out var window);
        ...
        return new RateLimitSnapshotView(StandardDimensions: standard, UnifiedStatus: unifiedStatus,
            UnifiedResetAt: unifiedResetAt, UnifiedWindows: windows, RepresentativeClaim: representativeClaim,
            RawHeaders: raw);
    }
    ...
    {
        return raw.TryGetValue(key: headerName, value: out var value) ? value : null;
    }

    /// <summary>
    /// Reads and parses <paramref name="headerName"/> as an integer, or <see langword="null"/> if absent or
    /// malformed.
    /// </summary>
    private static long? ReadLong(IReadOnlyDictionary<string, string> raw, string headerName)
    {
        return raw.TryGetValue(key: headerName, value: out var value) ? ParseLong(value) : null;
    }

    /// <summary>
    /// Reads and parses <paramref name="headerName"/> as an RFC 3339 date, or <see langword="null"/> if absent or
    /// malformed.
    /// </summary>
    private static DateTimeOffset? ReadDate(IReadOnlyDictionary<string, string> raw, string headerName)
    {
        return raw.TryGetValue(key: headerName, value: out var value) ? ParseDate(value) : null;
    }

    /// <summary>
    /// Resolves an OpenAI <c>x-ratelimit-reset-*</c> header (a Go-style duration relative to the response) into an
    /// absolute instant, or <see langword="null"/> when the header is absent, malformed, or <paramref name="observedAtUtc"/>
    /// wasn't supplied.
    /// </summary>
    private static DateTimeOffset? ReadGoDurationResetDate(IReadOnlyDictionary<string, string> raw, string headerName,
        DateTimeOffset? observedAtUtc)
    {
        if (!raw.TryGetValue(key: headerName, value: out var value) || observedAtUtc is not { } observed) return null;
    ...
    {
        return long.TryParse(s: value, style: NumberStyles.Integer, provider: CultureInfo.InvariantCulture,
    ...
    {
        return DateTimeOffset.TryParse(input: value, formatProvider: CultureInfo.InvariantCulture,
            styles: DateTimeStyles.RoundtripKind, result: out var parsed)
            ? parsed
            : null;
    }
    ...
    private static TimeSpan? ParseGoDuration(string value)
    {
        ...
        {
            if (!double.TryParse(s: match.Groups[1].Value, style: NumberStyles.Float,
                    provider: CultureInfo.InvariantCulture, result: out var amount)) return null;
