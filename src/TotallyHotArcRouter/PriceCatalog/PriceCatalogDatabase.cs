...
public sealed class PriceCatalogDatabase
{
    ...
    public PriceCatalogDatabase(IOptions<StorageOptions> storageOptions)
    {
        ...
        DatabasePath = storageOptions.Value.ResolveDatabasePath();
    }
    ...
    public bool EnsureCreated()
    {
        ...
        var alreadyExisted = File.Exists(DatabasePath);
    ...
    {
        if (ColumnExists(connection: connection, table: "aggregator_sources", column: "enabled")) return;
    ...
    {
        if (ColumnExists(connection: connection, table: "provider_endpoint_capabilities",
                column: "json_schema_response_format")) return;
    ...
    {
        if (ColumnExists(connection: connection, table: "model_prices", column: "cache_write_input_price")) return;
    ...
    {
        if (!ColumnExists(connection: connection, table: "provider_spend", column: "cache_creation_tokens"))
        {
            using var alterCreation = connection.CreateCommand();
            alterCreation.CommandText =
                "ALTER TABLE provider_spend ADD COLUMN cache_creation_tokens INTEGER NOT NULL DEFAULT 0;";
            alterCreation.ExecuteNonQuery();
        }

        if (!ColumnExists(connection: connection, table: "provider_spend", column: "cache_read_tokens"))
        {
            using var alterRead = connection.CreateCommand();
            alterRead.CommandText =
                "ALTER TABLE provider_spend ADD COLUMN cache_read_tokens INTEGER NOT NULL DEFAULT 0;";
            alterRead.ExecuteNonQuery();
        }

        if (!ColumnExists(connection: connection, table: "provider_spend", column: "last_usage_at"))
        {
    ...
    {
        if (ColumnExists(connection: connection, table: "model_prices", column: "is_approximate")) return;
    ...
    {
        if (!ColumnExists(connection: connection, table: "provider_budgets", column: "window_kind"))
        {
            using var alterKind = connection.CreateCommand();
            alterKind.CommandText =
                "ALTER TABLE provider_budgets ADD COLUMN window_kind TEXT NOT NULL DEFAULT 'Monthly';";
            alterKind.ExecuteNonQuery();
        }

        if (!ColumnExists(connection: connection, table: "provider_budgets", column: "window_hours"))
        {
    ...
    private static void SeedKnownSources(SqliteConnection connection)
    {
        foreach (var source in PriceCatalogOptions.KnownSources)
        {
            ...
            seed.CommandText = """
                               ...
                               """;
            seed.Parameters.AddWithValue(parameterName: "$name", value: source.Name);
            seed.Parameters.AddWithValue(parameterName: "$priority", value: source.DefaultPriorityScore);
            seed.Parameters.AddWithValue(parameterName: "$enabled", value: source.DefaultEnabled ? 1 : 0);
