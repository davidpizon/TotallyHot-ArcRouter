...
/// scope"). Its JSON is a flat object of
/// <c>
/// { modelName: { input_cost_per_token, output_cost_per_token,
/// litellm_provider, ... } }
/// </c>
/// , plus a <c>sample_spec</c> documentation sentinel that is not a model.
/// </remarks>
public sealed class LiteLlmPriceSourceClient : IPriceSourceClient
{
    ...
    public IReadOnlyList<NormalizedPrice> Normalize(JsonElement root)
    {
        ...
        {
            if (string.Equals(a: entry.Name, b: SampleSpecKey, comparisonType: StringComparison.Ordinal)) continue;
            ...
            if (!model.TryGetProperty(propertyName: "litellm_provider", value: out var providerElement) ||
            ...
            var standardInput = ReadPerMillion(model: model, fieldName: "input_cost_per_token");
            var standardOutput = ReadPerMillion(model: model, fieldName: "output_cost_per_token");
            ...
            prices.Add(new NormalizedPrice(
                ...
                CachedInputPrice: ReadPerMillion(model: model, fieldName: "cache_read_input_token_cost"),
                BatchInputPrice: ReadPerMillion(model: model, fieldName: "input_cost_per_token_batches"),
                BatchOutputPrice: ReadPerMillion(model: model, fieldName: "output_cost_per_token_batches"),
                CacheWriteInputPrice: ReadPerMillion(model: model, fieldName: "cache_creation_input_token_cost")));
        }

        if (skipped > 0)
            _logger.LogInformation(
                message: "LiteLLM normalization skipped {SkippedCount} unmappable entries, kept {KeptCount}.",
                skipped,
                prices.Count);
    ...
    {
        if (!model.TryGetProperty(propertyName: fieldName, value: out var element) ||
