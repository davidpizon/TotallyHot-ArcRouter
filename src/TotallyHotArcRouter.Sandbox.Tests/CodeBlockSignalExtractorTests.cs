...
{
    private static CodeBlockSignalExtractor CreateExtractor(SandboxOptions? options = null) =>
        new(
            new KeywordDimensionInferrer(),
            Options.Create(options ?? new SandboxOptions()),
    ...
    public void Extract_PythonBlock_ReturnsRequest()
    {
        ...
        Assert.Equal(SandboxLanguage.Python, request!.Language);
    ...
    public void Extract_PrefersRecognizedLanguageBlock()
    {
        ...
        Assert.NotNull(request);
        Assert.Equal(SandboxLanguage.JavaScript, request!.Language);
    }

    [Fact]
    public void Extract_RespectsMaxCodeBytes()
    {
        var extractor = CreateExtractor(new SandboxOptions { MaxCodeBytes = 5 });
        var response = "```python\n0123456789\n```";
    ...
    {
        var extractor = CreateExtractor(new SandboxOptions { MaxCodeBlocks = 0 });
        var response = "```python\nprint('hi')\n```";
