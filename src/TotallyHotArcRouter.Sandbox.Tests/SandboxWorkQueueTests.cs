...
public class SandboxWorkQueueTests
{
    private static SandboxRequest Request() =>
        new("code", SandboxLanguage.Python, "code_generation", "gpt-5.4", "corr", "sess");

    [Fact]
    public void TryEnqueue_WithinCapacity_Succeeds()
    {
        var queue = new SandboxWorkQueue(Options.Create(new SandboxOptions { QueueCapacity = 4 }));
    ...
    {
        var queue = new SandboxWorkQueue(Options.Create(new SandboxOptions { QueueCapacity = 2 }));
    ...
    {
        var queue = new SandboxWorkQueue(Options.Create(new SandboxOptions { QueueCapacity = 16 }));
