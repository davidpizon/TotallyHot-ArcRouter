...
public sealed class CodeBlockSignalExtractor : ISignalExtractor
{
    ...
    /// <param name="options">The sandbox options (size/count caps).</param>
    ...
    /// <inheritdoc />
    public SandboxRequest? Extract(SignalExtractionContext context)
    {
        ...
        var language = SandboxLanguage.Unknown;
        foreach (var block in blocks)
        {
            var lang = SandboxLanguages.FromHint(block.LanguageHint);
            if (lang != SandboxLanguage.Unknown)
            {
                chosen = block;
                language = lang;
                break;
