...
4. Re-run the exact verification done when this item was filed, to confirm progress and that neither
   assembly regresses:
   ```
   dotnet test src/TotallyHotArcRouter.Tests/TotallyHotArcRouter.Tests.csproj --configuration Release --coverage --coverage-output-format cobertura --coverage-settings coverage.runsettings --coverage-output coverage.cobertura.xml --results-directory TestResults/TotallyHotArcRouter.Tests
   dotnet test src/TotallyHotArcRouter.Sandbox.Tests/TotallyHotArcRouter.Sandbox.Tests.csproj --configuration Release --coverage --coverage-output-format cobertura --coverage-settings coverage.runsettings --coverage-output coverage.cobertura.xml --results-directory TestResults/TotallyHotArcRouter.Sandbox.Tests
