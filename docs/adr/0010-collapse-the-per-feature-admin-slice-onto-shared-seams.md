...
Three of the slice's layers are pure ceremony: the 11 `*AdminException` subclasses add **no state or
behavior** over `GrpcAdminException`; 13 of 16 GUI stores re-implement an identical
...
Collapsing these removes 11 `public` types from `Gui.Telemetry` and adds a registration interface to
...
- **Option 3 — Collapse each repeated layer onto a shared seam.** Delete the 11 exception subclasses
  in favor of `GrpcAdminException`; introduce `AdminStoreBase<TClient, TException>` as a template
  method for the 13 stores; introduce `IAdminServiceModule`, implemented by the `*AdminDependencies`
...
`AdminStoreBase` is generic over its exception type (`AdminStoreBase<TClient, TException>`) rather
than catching a common base. This is the decisive detail for ADR-0007: gRPC stores bind
`GrpcAdminException` and the two HTTP-backed stores (`ProviderAdminStore`, `UsageStore`) bind
`ProviderAdminException`, so both transports get the same scaffolding **without** a shared exception
hierarchy implying a shared transport.
...
- Good, because ~800 production lines of ceremony and 11 public types disappear, and the
  always-raise-`Changed`-even-on-failure rule — currently re-implemented 13 times, and the thing that
  keeps the UI off a permanent "loading" state when the router is down — gets one home and one test.
