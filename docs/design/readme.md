...
- **Backgrounds**: flat solid fills only. No photography, no illustration, no gradients, no textures/patterns, no grain. The one exception is a subtle radial-free flat `rgba` overlay + blur behind modals (`rgba(18,18,18,0.7)` + 2px blur) — functional dimming, not decorative.
