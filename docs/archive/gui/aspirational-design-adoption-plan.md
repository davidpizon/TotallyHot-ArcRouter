...
- Phase 5 | Status: Partial | — | — | Coverage/CI gate for the Gui project is not enforced in CI - the `windows-gui-build-and-test` job that would enforce it is deliberately disabled (`if: false`) | See [`phase5-conformance-checklist.md`](phase5-conformance-checklist.md); get explicit design/engineering sign-off
