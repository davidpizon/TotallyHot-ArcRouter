...
The dashboard is a **mix of live and mock data**, and which surface is which changes as telemetry lands — `docs/gui/dashboard.md`'s "Current status" section and `docs/gui/backlog.md` are the authority, deliberately not restated here. This design system's mock data mirrors the repo's own `MockData` fixtures, so it stays a faithful stand-in for whichever surfaces are still demo-backed.
...
- **Casing**: Title Case for headers and tab labels ("Sessions", "Cost Analytics", "Model Distribution", "Governance", "Console"); UPPERCASE for short status tags ("OK", "WARNING", "CRITICAL", "LIVE"); sentence case for routing-log messages and mock request/response text.
...
- **Animation**: every animation is attributable to a state change — but "attributable" is not the same as "absent". `docs/gui/MOTION.md` is the authority; the shape of it:
  - **Semantic entrances exist and are load-bearing.** Overlay Rise on modals (`.overlay-backdrop`/`.overlay-panel` carry it in CSS — a hand-rolled backdrop loses it), Panel Crossfade on tab switch, Row Enter with a first-mount stagger on conversation cards, Disclosure Expand, Toast Enter, and the drag list's Reorder Settle / Lift Detach.
  - **No overshoot in chrome.** Bounce and elastic curves are forbidden for UI: things arrive and stop. Data visualization is deliberately louder — the ECharts cost charts use staggered entrances, elastic donut growth, and rippling alert markers, because there the motion *is* the data signal.
...
- **Press/active states**: split by element type. CTA buttons carry a real press effect — `.btn:active:not(:disabled)` shrinks to `transform: scale(0.98)` for tactile feedback (`docs/gui/DESIGN.md` §4.2). Everything else interactive (icon actions, tab buttons, links, cards) has no `:active` rule at all, so press reads as an immediate application of the hover state. Do not add a scale to that second group — the shrink is the CTA's signature, and spreading it would spend the signal.
- **Borders**: 1px solid is the *only* structural depth cue — every card, input, and divider is a 1px `#4d4d4d` (`--border-button`) or semantic-tinted border. Note the markup class is `.border-slate-700`: it survives from the pre-adoption palette and was *recolored* to `#4d4d4d` in the override block, so the class name no longer matches stock Tailwind's slate-700 — trust the token, not the class name (`docs/gui/DESIGN.md` §2). No inner/outer shadows, no elevation system, no colored left-border-only card convention (cards that do get a colored edge, like agent-tinted turn cards, tint the *entire* border plus a slightly heavier 3px left edge — never a plain white/neutral card with only a colored strip).
...
- **Transparency/blur**: used exactly once, functionally — the modal backdrop (see Backgrounds above for the exact values) to focus attention on the dialog. Not used decoratively on cards or panels.
- **Layout**: the whole app is a fixed, non-scrolling `100vh` shell; individual panels scroll internally. A draggable split-pane divider (Sessions) is the one resizable layout element, clamped 20-65%.
