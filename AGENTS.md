...
- **Every class and function must have accurate XML documentation.** All types (classes, interfaces, records, enums, structs) and all methods/properties need a `///` doc comment that reflects current behavior — stale docs (describing removed parameters, old behavior, or placeholder text like "this class will contain...") are treated the same as missing docs. Follow the codebase's established convention: multi-sentence `<summary>` explaining *why*, not just what; `<inheritdoc/>` for interface implementations; type-level `<param>` tags for primary-constructor records; one-line summaries are acceptable for plain DTOs. Enforced at the compiler level for `TotallyHotArcRouter`, `TotallyHot.ArcRouter.Quality`, `TotallyHot.ArcRouter.Gui`, and `TotallyHotArcRouter.Gui.Admin/.Charts/.Components/.Console/.Telemetry/.Web`, and `TotallyHotArcRouter.Tray.Core`/`.Tray`: each sets `GenerateDocumentationFile`, so a missing doc on a public/protected member raises `CS1591`, which `TreatWarningsAsErrors` turns into a build failure. Only accuracy on update is not machine-checked — a stale-but-present doc still compiles, so review docs for correctness whenever you change the code they describe.
- **New GUI windows build on `DialogShell`.** Every new window, modal, or dialog in `TotallyHot.ArcRouter.Gui.Components` wraps its body in [`src/TotallyHotArcRouter.Gui.Components/Components/DialogShell.razor`](src/TotallyHotArcRouter.Gui.Components/Components/DialogShell.razor) rather than hand-rolling chrome. `DialogShell` is the extracted, shared implementation of the "System Settings" pattern: it supplies the `.overlay-backdrop`/`.overlay-panel` skeleton (which carry the entrance animation and the dynamic max-width/max-height sizing), the blurred backdrop and backdrop-click dismissal, the header bar with an uppercase `text-sm` title and an `x` close glyph, and the scrollable `.overlay-content` body. Pass `Title`, `CloseAriaLabel`, and `OnClose` (all required — the shell never closes itself, the caller decides what closing means); pass `ContentClass` only to vary the body's `space-y-*` gap, and `EnableEscapeToClose` only for a dialog that genuinely wants Escape. Every dialog in the app is already on it — `SettingsModal.razor`, `ProviderEditDialog.razor`, `RemoveProviderDialog.razor`, `UnlockSecretFieldDialog.razor`; `UnlockSecretFieldDialog.razor` is the smallest example to copy. Do not reintroduce a hand-written backdrop/panel: it silently loses the entrance animation and the content-scroll behavior. Deviate only where content genuinely requires it (e.g. a wider panel for a table, via a CSS override rather than an inline `max-w-*`) and never on the header, dismissal behavior, or close API. Full contract: [`docs/gui/DESIGN.md`](docs/gui/DESIGN.md) §4.1.
